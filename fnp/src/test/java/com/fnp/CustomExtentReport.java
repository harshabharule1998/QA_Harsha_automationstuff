...
public class CustomExtentReport {
	...
	private ExtentPDFReporter extentPdfReporter;
	...
	public void generateExtentReport() throws InterruptedException {
		...
		if (this.reportDir != null && !this.reportDir.isEmpty() && this.testResults != null) {
			...
			extentPdfReporter = new ExtentPDFReporter(reportDir);
			extentReports.attachReporter(extentPdfReporter);

			// extentSparkReporter = new ExtentSparkReporter(reportDir);

			extentSparkReporter = new ExtentSparkReporter(reportDir).filter().statusFilter()
			...
			scenarioResults.forEach((scenarioResult) -> {
				...
				// 6. Using the same scenario object, we will get the info about
				// the scenario
				...
				// 8. loop over the step result list, get the info about
				// scenario step and its execution status

				scenarioResult.getStepResults().forEach((step) -> {

					// Adding the failing Scenario Step with Scenario node

					if (step.getResult().getStatus().equals("failed")) {
	...
	private ExtentTest createFeatureNode(String featureName, String featureDesc) {
		// if the title of feature is same, I will return same instance of
		// extent test
	...
	private ExtentTest createScenarioNode(ExtentTest featureNode, String scenarioTitle) {
		// if the title of scenario is same, I will return same instance of
		// extent test
		// else I will create a new instance and then return it
	...
	private void setConfig() {
		...
		extentPdfReporter.config().setTitle(reportTitle);
